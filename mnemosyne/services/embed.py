...
def _embed(text: str) -> list[float]:
    ...
    # Ollama returns {'embeddings': [[...]]} for a single input, so extract the first vector
    if isinstance(response.embeddings, list) and len(response.embeddings) == 1 and isinstance(response.embeddings[0], list):
